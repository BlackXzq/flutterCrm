...
class PageScrollPhysics extends ScrollPhysics {
  ...
  final double extraChildWidth;
  final double pageMargin;
  final double scale;
  PageScrollPhysics(
      {ScrollPhysics parent,
      this.pageWidth,
      this.extraChildWidth,
      this.pageMargin,
      this.scale})
      : super(parent: parent);
  ...
  bool isBeyondRegion(double pixel) {
    UserUtils.getHomeItemsCount().then((count) {
      childCount = count;
    });
    if (childCount == null) {
      return false;
    ...
    return pixel >=
        (childCount - 2) * pageWidth * scale + extraChildWidth - pageMargin * 2;
  }

  @override
  String toString() {
    return 'PageScrollPhysics{childCount: $childCount, pageWidth: $pageWidth, extraChildWidth: $extraChildWidth, pageMargin: $pageMargin, scale: $scale}';
  }

  @override
  double applyPhysicsToUserOffset(ScrollMetrics position, double offset) {
//    print('AAAAA ======> ${position.pixels}');
//    if (isBeyondRegion(position.pixels) && offset < 0) {
//      return 0;
//    }
  ...
  Future<int> getCount() async {
    return await UserUtils.getHomeItemsCount();
  }
  ...
  PageScrollPhysics applyTo(ScrollPhysics ancestor) {
    return PageScrollPhysics(
        ...
        pageWidth: pageWidth,
        extraChildWidth: extraChildWidth,
  ...
  Simulation createBallisticSimulation(
    ...
    if (isBeyondRegion(position.pixels)) {
      return null;
    }
    ...
    if (target != position.pixels && !isBeyondRegion(target))
...
class _PageViewState extends State<CustomPageView> {
  ...
  Widget build(BuildContext context) {
    print(
        'widget.itemCount ${widget.itemCount} widget.extraWidth ${widget.extraWidth} widget.pageWidth +${widget.pageWidth} widget.scale ${widget.scale}');
    if (_kPagePhysics == null) {
      _kPagePhysics = new PageScrollPhysics(
          extraChildWidth: widget.extraWidth,
          pageWidth: widget.pageWidth,
          pageMargin: widget.pageMargin,
          scale: widget.scale);
    }
    final AxisDirection axisDirection = _getDirection(context);
    ScrollPhysics physics = _kPagePhysics;
